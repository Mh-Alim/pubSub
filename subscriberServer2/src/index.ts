...
app.post("/subscribe", (req: any, res: any) => {
  ...
  subscribers[topic].push((message: string) => {
    console.log(`Server2 received messaged on ${topic}, msg: ${message}`);
  });
  res.status(200).json({ message: `Server2 Subscribed to topic: ${topic}` });
});


app.listen(5000, () => {
  console.log("Subscriber is running on port 5000");
});
